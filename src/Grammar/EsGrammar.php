...
use Zwen\SqlDsl\Processors\DefaultProcessor;
use Zwen\SqlDsl\Positions\PositionCalculator;
...
class EsGrammar implements Grammar {
    ...
    private $isscroll=0;
    public $scroll;
    public $agg_size = 1000000;//分组操作最大返回100w

    public function sql2dsl($sql)
    {
        //解析sql
        $this->parsed = (new DefaultProcessor()) ->process($sql);
            ...
            if(isset($arr[$i-1])){
                ...
                if($countmp==0){
                    if(!isset($arr[$i][$key_arr[0]]['date_histogram'])){
                        ...
                        $arr[$i][$key_arr[0]]['terms']['size'] = $this->agg_size;
                        if($order){
                ...
                if(count($arr)==1 && $countmp==0){
                    ...
                    if(!isset($arr[$i][$key_arrs[0]]['date_histogram'])){
                        ...
                        $arr[$i][$key_arrs[0]]['terms']['size'] = $this->agg_size;
                        if($order){
    ...
    private function groupby($arr){
        ...
        for($i=0; $i <count($arr); $i++) {
            ...
            $agg[$i][$termk_tmp]['terms']['size'] = $this->agg_size;
        }
